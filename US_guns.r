...
write.csv(all_death, "summary_output.csv", row.names=F)
...
make_graph = function(gender, intent, nudge, y1, y2, y3) {
  ...
  df = all_death %>%
    # filter to a specific gender and intent
    filter(Gender == gender, Injury.Intent == intent) %>%
    # add in label locations (made manually)
    merge(read.csv(paste0("labels/",lower.gender,"_",lower.intent,".csv"))) %>%
    # sort so large circles are behind small circles
    arrange(desc(Population)) %>%
    # labels outside a circle are the opposite color
    mutate(Color = ifelse(Color == "light.color", light.color, dark.color))
  
  # get the trendline and stats
  ...
  # individual graph with state labels
  g2 = g1 +
    # make y-axis labels line up with other graphs
    ...
    # allow notes to go below plot area
